...
module.exports = function (entry) {
  var mockerList = business.getMockerList(entry.MOCKER_PATH);
  ...
  router.get('/sys-cgi/mocker', function (req, res) {
  ...
  router.get('/sys-cgi/mocker/:mockerName', function (req, res) {
    var result = business.getMocker(entry.MOCKER_PATH, req.params.mockerName);

    res.jsonp(result);
  });

  // GET /sys-cgi/mocker/:mockerName/readme 获得这个 mocker 的readme信息
  router.get('/sys-cgi/mocker/:mockerName/readme', function (req, res) {
  ...
  router.post('/sys-cgi/mocker/:mockerName', function (req, res) {
    var result = business.updateMocker(entry.MOCKER_PATH, req.params.mockerName, req.body);
  ...
  mockerList.forEach(function (mockerData) {
    // 默认是 get 请求，除非定义 method 字段
    var METHOD = (mockerData.method || 'get').toLowerCase();
    var ROUTE_PATH = mockerData.route;
    ...
    router[METHOD](ROUTE_PATH, function (req, res, next) {
      ...
      var mockerBasePath = entry.MOCKER_PATH;

      // 从请求 req 或者 config.json 文件中检查当前请求是否需要禁用 mock 服务
      var isDisable = req.query._m_disable || req.body._m_disable;
      if (!isDisable) {
        // 此处要重新获取新的数据，以便取到缓存的。
        // TODO 此处还可以优化，比如及时更新缓存中的数据，而不需要每次都去获取
        var curMockerData = business.getMocker(mockerBasePath, mockerData.name);
      ...
      if (isDisable) {
        ...
        // 请求
        business.getMockModule(mockerBasePath, url, params, req).then(function (result) {
