...
var HandlerParser = function () {
  ...
  HandlerParser.prototype.getHandler = function getHandler(handlerName, isReset) {
    ...
    //===============================================================
    var curDefinedHandler = this.definedHandlers.filter(function (item) {
  ...
  HandlerParser.prototype.getReqInfoByRoute = function getReqInfoByRoute(route) {
    ...
    // TODO 此处还需要进一步优化
    var requiredModule = this.definedHandlers.filter(function (item) {
      return item.name === handlerInfo.name;
    })[0].handleModules.filter(function (item) {
      return item.name === handleModuleInfo.name;
    })[0].module;
