...
describe('check config/CoverageConfig.js', () => {
  describe('check constructor(opts, basePath, tag)', () => {
    ...
    it('if opts is String(relative path) and tag is undefined', () => {
      expect(new CoverageConfig(matmanConfig, './is/relative/file.json', caseModuleFilePath)).to.eql({
    ...
    it('if opts is String(relative path) and tag is "mytag"', () => {
      expect(new CoverageConfig(matmanConfig, './is/relative/file.json', caseModuleFilePath, 'mytag')).to.eql({
    ...
    it('if opts is String(absolute path) and tag is "mytag"', () => {
      expect(new CoverageConfig(matmanConfig, '/is/absolute/file.json', caseModuleFilePath, 'mytag')).to.eql({
    ...
    it('if opts is Object(relative path) and tag is undefined', () => {
      expect(new CoverageConfig(matmanConfig, { path: './is/relative/file.json' }, caseModuleFilePath)).to.eql({
        path: path.join(matmanConfig.coveragePath, './is/relative/file.json'),
        tag: undefined,
      });
    });
    ...
    it('if opts is Object(absolute path) and tag is undefined', () => {
      expect(new CoverageConfig(matmanConfig, { path: '/is/absolute/file.json' }, caseModuleFilePath)).to.eql({
        path: '/is/absolute/file.json',
        tag: undefined,
      });
    });
  ...
  describe('check getPathWithId(id)', () => {
    ...
    it('check tag is "mytag" and id=1024', () => {
      expect(new CoverageConfig(matmanConfig, true, caseModuleFilePath, 'mytag').getPathWithId(1024)).to.equal(path.join(
        matmanConfig.coveragePath,
    ...
    it('check tag is undefined and id=1024', () => {
      expect(new CoverageConfig(matmanConfig, true, caseModuleFilePath).getPathWithId(1024)).to.equal(path.join(
        matmanConfig.coveragePath,
