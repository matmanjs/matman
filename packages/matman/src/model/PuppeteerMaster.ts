...
import {evaluate} from '../util/masterUtils';
...
class Puppeteer extends EventEmitter implements Master {
  ...
  async gotoPage(): Promise<void> {
    ...
    // 兼容性处理
    if (
      typeof this.pageDriver.nightmareWaitFn === 'number' ||
      typeof this.pageDriver.nightmareWaitFn === 'string'
    ) {
      await this.page?.waitFor(this.pageDriver.nightmareWaitFn as any);
    }
    // 函数执行结果给 waitFor
    if (typeof this.pageDriver.nightmareWaitFn === 'function') {
      await this.page?.waitFor(
        this.pageDriver.nightmareWaitFn(...this.pageDriver.nightmareWaitFnArgs) as any,
      );
    }
