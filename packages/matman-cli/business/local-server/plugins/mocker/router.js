...
module.exports = (router, entry) => {
  ...
  mockerList.forEach((mockerItem) => {
    ...
    switch (mockerConfig.plugin) {
      case 'xhr':
        dealPluginXhr(router, ROUTE_PATH, mockerItem, mockerParser);
        break;

      default:
        console.error('Unknown plugin!', mockerConfig);
        break;
    }
...
function dealPluginXhr(router, ROUTE_PATH, mockerItem, mockerParser) {
  const mockerConfig = mockerItem.config;
